#' Gof test using the Anderson-Darling test statistic and the gamma
#' distribution
#' 
#' \code{\link{gofRosenblattGamma}} contains the RosenblattGamma gof tests for
...
#' \code{\link{gofTstat}} from the package copula. It is possible to insert
...
#' via the distribution function of the gamma distribution. The \eqn{H_0}{H0}
#' hypothesis is \deqn{C \in \mathcal{C}_0}{C in Ccal0} with
#' \eqn{\mathcal{C}_0}{Ccal0} as the true class of copulae under \eqn{H_0}{H0}.
#' 
...
#' -> (0,1)^d}. Following Genest et al. (2009) ensures this transformation the
#' decomposition of a random vector \eqn{\mathbf{u} \in [0,1]^d}{u in [0,1]^d}
...
#' \eqn{E_i}{E[i]}, given by \deqn{E_1 = \mathcal{R}(U_1), \dots, E_n =
#' \mathcal{R}(U_n).}{E_1 = R(U_1), ..., E_n = R(U_n).} The mapping is
#' performed by assigning to every vector \eqn{\mathbf{u}}{u} for \eqn{e_1 =
#' u_1}{e[1] = u[1]} and for \eqn{i \in \{2, \dots, d\}}{i in {2, ..., d}},
#' \deqn{e_i = \frac{\partial^{i-1} C(u_1, \dots, u_i, 1, \dots, 1)}{\partial
#' u_1 \cdots \partial u_{i-1}} / \frac{\partial^{i-1} C(u_1, \dots, u_{i-1},
#' 1, \dots, 1)}{\partial u_1 \cdots \partial u_{i-1}}.}{e[i] = (d^(i-1)
#' C(u[1], ..., u[i], 1, ..., 1))/(d u[1] ... d u[i-1]) / (d^(i-1) C(u[1], ...,
#' u[i-1], 1, ..., 1))/(d u[1] ... d u[i-1]).}
#' 
...
#' (- \ln e_{ij})}{x[j] = -log(e_{1j})-...-log(e_{dj})},
#' \eqn{\Gamma_d()}{pgamma( . ,shape=d)} denotes the distribution function of
...
#' 
#' @export gofRosenblattGamma
gofRosenblattGamma <- function(copula = c("normal", "t", "clayton", "gumbel", 
  ...
  if (length(copula) > 1) {
    stop("'copula' has to be a vector of length 1. Please select only 
         one copula.")
  }
...
numbers."
  ...
  res <- try(.gofCopulapb(copula = erg[[1]], x = erg[[2]], M = M, 
                          method = "AnGamma", estim.method = "mpl", 
...
Therefore df.est was set to FALSE for the bootstrapping."
    ...
    res.f <- .gofCopulapb(copula = erg[[1]], x = erg[[2]], M = M, 
                          method = "AnGamma", estim.method = "itau", 
                          processes = processes, param.est = param.est, 
