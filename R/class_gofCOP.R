...
plot.gofCOP = function(x, copula = NULL, hybrid = NULL, point.bg = "white", point.col = "black", point.cex = .7,
                       jitter.val = 0.1, pal = "xmen", bean.b.o = .2, inf.method = "iqr", theme = 2, ...){
  if (class(x) == "gofCOP" & dim(x[[1]]$res.tests)[1] == 1) {stop("Plotting p-values is only supported for various tests. Please consider gofHybrid() and gof().")}
  if(!is.element(inf.method, c("hdi", "iqr", "sd", "se"))){stop("Please consider for the argument 'inf.method' either 'hdi', 'iqr', 'sd' or 'se'.")}
  ...
  for (j in 1:length(x)) {
    ...
    if(is.element(inf.method, c("sd", "se"))){
      ...
      cnt = sort(unique(cnt))
      if(length(cnt) == 1){warning(paste(paste("'hyb" , as.character(hybrid[cnt]), "'", sep = ""), "is removed from plotting as the chosen 'inf.method' is not applicable."))}
      if(length(cnt) > 1){warning(paste("The hybrids", paste(as.character(hybrid[cnt]), collapse = ","), "are removed from plotting as the chosen 'inf.method' is not applicable."))}
      if(length(cnt) > 0){hybrid = hybrid[-cnt]}
    }
    ...
    if(any(is.element(names(args), tabu_args))){stop("Please do not alter the arguments 'formula', 'data', 'sortx', 'ylim', 'xaxt', 'xlim' or 'ylab'.")}
    
    if((length(x) > 2) & (length(hybrid) > 2)){adj = 0.75}else{adj = 0.5}
    
    pirateplot(formula = pvalues ~ copulae + hybrids, data = toplot, theme = theme, sortx = "sequentiell", 
