gofco = function(copulaobject, x, tests = c("gofPIOSRn", "gofKernel"), customTests = NULL, margins = "ranks", 
  ...
  if(!is.null(tests) & any(!is.element(tests, gofTest4Copula(as.character(substr(class(copulaobject), 1, nchar(class(copulaobject))-6)), dim(x)[2]))) || !is.null(customTests) & any(!is.element(customTests, ls(".GlobalEnv")))) {stop("At least one of the tests in 'tests' is not implemented, cannot handle a dataset of this dimension or at least one of the tests in 'customTests' does not match any function in the global workspace. Please check if it is correctly spelled in the function call.")}
  ...
  switch(class(copulaobject),
         normalCopula = {copula = "normal"; param = copulaobject@parameters; param.est = if (is.na(copulaobject@parameters)){TRUE} else {FALSE}; df = 4; df.est = TRUE; dispstr = copulaobject@dispstr},
         tCopula = {copula = "t"; param = copulaobject@parameters[-length(copulaobject@parameters)]; param.est = if (is.na(copulaobject@parameters[1])){TRUE} else {FALSE}; df = copulaobject@parameters[length(copulaobject@parameters)]; df.est = if (copulaobject@df.fixed == FALSE) {TRUE} else if (copulaobject@df.fixed == TRUE) {FALSE}; dispstr = copulaobject@dispstr},
         claytonCopula = {copula = "clayton"; param = copulaobject@parameters; param.est = if (is.na(copulaobject@parameters)){TRUE} else {FALSE}; df = 4; df.est = TRUE},
         frankCopula = {copula = "frank"; param = copulaobject@parameters; param.est = if (is.na(copulaobject@parameters)){TRUE} else {FALSE}; df = 4; df.est = TRUE},
         gumbelCopula = {copula = "gumbel"; param = copulaobject@parameters; param.est = if (is.na(copulaobject@parameters)){TRUE} else {FALSE}; df = 4; df.est = TRUE},
         stop("The class of the object is not supported."))
  gofHybrid(copula = copula, x = x, tests = tests, customTests = customTests, margins = margins, dispstr = dispstr, M = M, param = param, param.est = param.est, df = df, df.est = df.est, m = m, MJ = MJ, delta.J = delta.J, nodes.Integration = nodes.Integration, seed.active = seed.active, processes = processes)
}