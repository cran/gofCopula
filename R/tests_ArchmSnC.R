#' The ArchmSnC test based on the Rosenblatt transformation for 
#' archimedean copula
#' 
#' \code{\link{gofArchmSnC}} contains the SnC gof test with a Rosenblatt 
...
#' D_n(\mathbf{u})}{n int_{[0,1]^d} ( {Dn(u) - Cind(u)}^2 )dDn(u)} with
...
#' 
#' @export gofArchmSnC
gofArchmSnC <- function(copula = c("clayton", "gumbel", "frank", "joe", 
...
"'copula' has to be a vector of length 1. Please select only one copula."
  ...
  if (is.element(copula, c("clayton", "gumbel", "frank", "joe", 
                           "amh")) == FALSE) {
    stop("This copula is not implemented for gofArchmSnC.")
  }
...
numbers."
  ...
  res <- try(.gofCopulapb(copula = erg[[1]], x = erg[[2]], M = M, 
                          method = "ArchmSnC", estim.method = "mpl", 
...
Therefore df.est was set to FALSE for the bootstrapping."
    ...
    res.f <- .gofCopulapb(copula = erg[[1]], x = erg[[2]], M = M, 
                          method = "ArchmSnC", estim.method = "itau", 
                          processes = processes, param.est = param.est, 
