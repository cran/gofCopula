gofGetHybrid = function(result, p_values = NULL, nsets = NULL){
  ...
  if (length(nsets) > 1) {stop("'nsets' has to be a single integer entry.")}
  ...
  for (j in 1:length(result)) {
    ...
    if(length(result[[j]]) == 7){
      ...
      }else{res_length = length(tmp)}
    }else{
      res_length = 1}
    
    num_tests = length(p_values) + res_length
    if(num_tests <= 1){stop("The input should contain information of at least two different tests.")}
    if(!is.null(nsets)){if(nsets < 1 | nsets > num_tests){stop("Please set nsets larger or equal than 1 and smaller or equal than the number of single tests. Otherwise hybrid testing is not meaningful.")}}
    
    ##getting combinations
    which_comb = list()
    for(i in 1:(2^num_tests)){
    ...
    if (!is.null(nsets)){
      if (nsets > 1) {
        index = which(lapply(comb_exist, FUN = function(x){length(x) == nsets}) == TRUE)
        comb_wanted = comb_exist[index]
        for (i in 1:length(comb_wanted)){
    ...
    res_list[[j]] = list(method = "Hybrid test p-values for given single tests.",
