...
#' distribution
#' 
#' \code{\link{gofRosenblattChisq}} contains the RosenblattChisq gof test for
...
#' 
#' @export gofRosenblattChisq
gofRosenblattChisq <- function(copula = c("normal", "t", "clayton", "gumbel", 
...
numbers."
  ...
  res <- try(.gofCopulapb(copula = erg[[1]], x = erg[[2]], M = M, 
                          method = "AnChisq", estim.method = "mpl", 
...
Therefore df.est was set to FALSE for the bootstrapping."
    ...
    res.f <- .gofCopulapb(copula = erg[[1]], x = erg[[2]], M = M, 
                          method = "AnChisq", estim.method = "itau", 
                          processes = processes, param.est = param.est, 
