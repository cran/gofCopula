...
#' 
#' \code{\link{gofCvM}} performs the \code{"CvM"} gof test, described in Genest
...
#' with the function \code{\link{gofTstat}} from the package copula. It is
...
#' \code{"plackett"}. The parameter estimation is performed with pseudo
...
#' \eqn{H_0}{H0}. The test statistic \eqn{T} is then defined as
#' 
#' \deqn{T = n \int_{[0,1]^d} \{ C_n(\mathbf{u}) - C_{\theta_n}(\mathbf{u})
#' \}^2 d C_n(\mathbf{u})}{n int_{[0,1]^d} ( {Cn(u) - Cthetan(u)}^2 )dCn(u)}
#' with \eqn{C_{\theta_n}(\mathbf{u})}{Cthetan(u)} the estimation of \eqn{C}
...
#' @references Rosenblatt, M. (1952). Remarks on a Multivariate Transformation.
...
#' 
#' @export gofCvM
gofCvM <- function(copula = c("normal", "t", "clayton", "gumbel", "frank", 
                             "joe", "amh", "galambos", "huslerReiss", "tawn", 
                             "tev", "fgm", "plackett"), 
                  x, param = 0.5, param.est = TRUE, 
                  df = 4, df.est = TRUE, margins = "ranks", flip = 0, 
                  M = 1000, 
...
"'copula' has to be a vector of length 1. Please select only one copula."
  ...
  if (is.element(copula, c("normal", "gaussian", "t", "clayton", "gumbel", 
    ...
    stop("This copula is not implemented for gofCvM.")
  }
  if (is.element(copula, c("amh", "galambos", "huslerReiss", "tawn", "tev", 
                           "fgm", "plackett")) & dim(x)[2] > 2) {
