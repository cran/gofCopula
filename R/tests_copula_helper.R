.gofCopulapb = function (copula, x, M = 1000, method = eval(formals(.Tstats)$Tstat), estim.method = eval(formals(fitCopula)$method), processes, param.est, df.est, dispstr, param.margins, margins, seed.active, ...) {
  #required to avoid error on global variable definition
  cnt = NULL
  
  if (!is.null(seed.active)) {set.seed(seed.active[1], kind = "default")}
  bs.ac.c = list()
  ...
  ac.c <- if (method == "Sn") {
    if(as.character(substr(class(copula), 1, nchar(class(copula))-6)) == "t"){
  ...
  opts = list(progress = progBar)
    ...
    bs.ac.c = foreach(cnt = 1:M, .options.snow = opts) %dopar% {
      progBar(cnt)
      seed.counter = 0
      repeat {
        ...
        copula.sim = try(.margins.param.est(copula = as.character(substr(class(copula), 1, nchar(class(copula))-6)), margins = NULL, x = xsim, 
      ...
      if (method == "Sn") {
        if(as.character(substr(class(copula.sim), 1, nchar(class(copula.sim))-6)) == "t"){
  ...
  res = structure(class = "gofCOP", 
            list(
                 ...
                 theta = if(class(copula) != "tCopula") {copula@parameters} else {copula@parameters[-length(copula@parameters)]},
                 df = if(class(copula) != "tCopula") {NULL} else {copula@parameters[length(copula@parameters)]},
                 res.tests = matrix(c(test, ac.c), ncol = 2,  
